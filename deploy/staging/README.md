...
After migration, run `deploy/staging/seed-staging.mjs` only for a fresh database
whose ledger exactly matches the migration files shipped by the reviewed source
(currently `124/124`) and has zero users. The seed derives this count from the
contiguous `lib/db/drizzle` ledger and refuses gaps or duplicate prefixes. It creates synthetic reference data and one synthetic
...
- the migration ledger exactly matches the reviewed source (currently `124/124`);
...
`fas_migrator@127.0.0.1:5432/fasos_staging`, requires the exact source-bound
ledger (currently `124/124`),
...
After the run, verify the ledger remains the exact source-bound count (currently
`124/124`), the ten core/social
...
- ledger count exactly matches the reviewed source (currently `124`);
