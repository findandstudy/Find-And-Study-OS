...
- Auth middleware checks **Bearer first**, then falls back to session. A valid Bearer sets `req.user` + `req.tokenScopes` (+ `req.apiTokenAuth=true`). Sessions are completely unaffected — no regression.
