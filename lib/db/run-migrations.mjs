...
function pnpmInvocation(args) {
  ...
  if (explicitCli) {
    if (
      ...
      throw new Error(
        "[migration] BLOCKED: FAS_REVIEWED_PNPM_CLI must be an absolute pnpm.cjs path",
      );
    }
    return {
...
const versionResult = runPnpm(["--version"], {
  cwd,
  encoding: "utf8",
});
if (
  ...
  console.error(
    `[migration] BLOCKED: reviewed migrations require pnpm ${EXPECTED_PNPM_VERSION}`,
  );
  process.exit(1);
}
...
if (result.error) {
  console.error("[migration] BLOCKED: pnpm execution failed");
}
