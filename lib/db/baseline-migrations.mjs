...
function collectExpectedCatalog(migrations) {
  ...
  return { tables, columns, indexes, constraints, enums };
}
...
async function assertSchemaReady(client, migrations) {
  ...
  const missingConstraints = [...expected.constraints].filter(
    (name) => !actualConstraints.has(name),
  );
...
export async function baselineMigrationLedger({
  ...
  try {
    ...
    return { databaseName, entries: migrations.length };
  } catch (error) {
    try {
      await client.query("ROLLBACK");
    } catch {}
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    ...
    console.log(
      `[migration-baseline] OK: ${result.entries} entries recorded for ${result.databaseName}`,
    );
  } catch (error) {
