-- Default-unwired command adapter for governed public web publication.
-- Defines capability vocabulary and an atomic, idempotent RPC. It grants no
-- role package, login role, route, worker or production rollout.
...
BEGIN
  FOR expected IN
    SELECT * FROM (VALUES
      ('public_web.content.read', 'Read tenant-scoped public web publication operations.', 'LOW', false, false, false),
      ('public_web.content.write', 'Create drafts and manage review-ready public web content.', 'MEDIUM', false, false, false),
      ('public_web.content.review', 'Review public web content independently from its author.', 'HIGH', false, false, false),
      ('public_web.content.publish', 'Publish or retire independently approved public web content.', 'CRITICAL', false, true, true),
      ('public_web.content.index', 'Change search index eligibility for published public web content.', 'HIGH', false, true, true)
    ) AS definition("key", "description", "risk_class", "delegable", "step_up_required", "approval_required")
  LOOP
    ...
    IF NOT FOUND THEN
      INSERT INTO capability_definitions (
        ...
        expected."key", expected."description", expected."risk_class",
        expected."delegable", expected."step_up_required",
        expected."approval_required", 'ACTIVE', 1
      );
    ELSIF existing."description" <> expected."description"
      OR existing."risk_class" <> expected."risk_class"
      OR existing."delegable" <> expected."delegable"
      OR existing."step_up_required" <> expected."step_up_required"
      OR existing."approval_required" <> expected."approval_required"
      OR existing."status" <> 'ACTIVE'
      OR existing."version" <> 1 THEN
      RAISE EXCEPTION 'public web capability conflicts with reviewed definition: %', expected."key";
    END IF;
  END LOOP;
END;
$$;

ALTER TABLE public.public_web_publication_receipts
  ADD COLUMN authorization_decision_receipt_id uuid NOT NULL,
  ADD COLUMN request_hash text NOT NULL,
  ADD CONSTRAINT public_web_publication_receipts_authorization_fk
    FOREIGN KEY (tenant_id, authorization_decision_receipt_id)
    REFERENCES public.access_decision_receipts(tenant_id, id)
    ON DELETE RESTRICT,
  ADD CONSTRAINT public_web_publication_receipts_request_hash_chk
    CHECK (request_hash ~ '^[0-9a-f]{64}$');

CREATE INDEX public_web_publication_receipts_authorization_idx
  ON public.public_web_publication_receipts (
    tenant_id, authorization_decision_receipt_id
  );

CREATE SCHEMA fas_public_web_v1;
REVOKE ALL ON SCHEMA fas_public_web_v1 FROM PUBLIC;

CREATE FUNCTION fas_public_web_v1.assert_scope(
  p_tenant uuid,
  p_organization uuid
) RETURNS void
LANGUAGE plpgsql
...
AS $$
DECLARE
  bound_tenant text := nullif(current_setting('app.tenant_id', true), '');
  bound_organization text := nullif(current_setting('app.organization_id', true), '');
BEGIN
  IF bound_tenant IS NULL
    OR bound_organization IS NULL
    OR bound_tenant <> p_tenant::text
    OR bound_organization <> p_organization::text
    OR NOT EXISTS (
      SELECT 1
    ...
    RAISE EXCEPTION 'public web command scope mismatch' USING ERRCODE = '42501';
  END IF;
END;
$$;

CREATE FUNCTION fas_public_web_v1.apply_publication_command(
  p_input jsonb
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
REVOKE ALL ON ALL FUNCTIONS IN SCHEMA fas_public_web_v1 FROM PUBLIC;

COMMENT ON SCHEMA fas_public_web_v1 IS
  'Default-unwired public web command facade. Function ownership and EXECUTE grants require a separate reviewed authority bootstrap.';

-- Deliberately absent: role-package grants, LOGIN roles, public/admin routes,
-- provider calls, bulk backfills, staging/production rollout and AI publish.
