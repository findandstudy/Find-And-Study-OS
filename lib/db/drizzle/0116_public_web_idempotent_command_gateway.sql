-- Additive idempotent gateway for 0114/0115. The gateway takes the same
-- per-content advisory lock before replay detection, uses explicitly prefixed
-- variables and delegates only genuinely new commands to the mutation RPC.

CREATE FUNCTION fas_public_web_v1.apply_publication_command_v2(
  p_input jsonb
) RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
-- The reviewed executor bootstrap must grant only v2; v1 stays ungranted.
