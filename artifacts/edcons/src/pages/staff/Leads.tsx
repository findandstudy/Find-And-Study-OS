...
/* ── LeadCard ──────────────────────────────────────────────── */
function LeadCard({ lead, onView, onMessage, onEmail, showRevenue, variant, assignedUserName, onAssignToMe, isAdmin }: {
  lead: any; onView: (id: number) => void; onMessage?: (lead: any) => void; onEmail?: (lead: any) => void; showRevenue: boolean; variant?: ColVariant;
    ...
    >
      ...
      <div className="px-4 pb-3 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5">
          {onMessage && (
            <button onClick={(e) => { e.stopPropagation(); onMessage(lead); }} title="Message"
          ...
          {onEmail && lead.email && (
            <button onClick={(e) => { e.stopPropagation(); onEmail(lead); }} title="Email"
              className="w-6 h-6 flex items-center justify-center rounded-md text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors">
          ...
          {!lead.assignedToId && onAssignToMe && (
            <button onClick={(e) => { e.stopPropagation(); onAssignToMe(lead.id); }} title="Assign"
              className="w-6 h-6 flex items-center justify-center rounded-md text-muted-foreground hover:text-primary hover:bg-primary/10 transition-colors">
...
/* ── DroppableColumn ──────────────────────────────────────── */
function DroppableColumn({ col, leads, showRevenue, onView, onMessage, onEmail, staffUsersMap, onAssignToMe, isAdmin }: {
  col: ColDef; leads: any[]; showRevenue: boolean; onView: (id: number) => void;
  onMessage?: (lead: any) => void; onEmail?: (lead: any) => void;
    ...
    <div className={`w-72 flex flex-col max-h-full rounded-2xl border overflow-hidden ${colBg}`}>
      ...
      <div ref={setNodeRef} className={`p-3 flex-1 overflow-y-auto custom-scrollbar transition-colors duration-150 ${dropBg}`}>
        <SortableContext items={leads.map((l) => l.id)} strategy={verticalListSortingStrategy}>
          {leads.map((lead) => (
            <LeadCard key={lead.id} lead={lead} onView={onView} onMessage={onMessage} onEmail={onEmail} showRevenue={showRevenue} variant={v} assignedUserName={lead.assignedToId && staffUsersMap ? staffUsersMap[lead.assignedToId] : undefined} onAssignToMe={onAssignToMe} isAdmin={isAdmin} />
          ))}
          {leads.length === 0 && (
                ...
                {columns.map((col) => {
                  ...
                  return (
                    <DroppableColumn
                      ...
                      onMessage={(lead) => setLocation(`/staff/messages`)}
                      onEmail={(lead) => { window.open(`mailto:${lead.email}`, "_blank"); }}
