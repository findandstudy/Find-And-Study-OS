        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <span className="inline-flex items-center gap-2 bg-accent/10 text-accent text-sm font-semibold px-4 py-2 rounded-full mb-6">
              <BookOpen className="w-4 h-4" /> The EduCons Blog
            </span>
            <h1 className="text-4xl md:text-6xl font-display font-bold text-foreground mb-6">
              Insights & <span className="text-accent">Advice</span>
            </h1>
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto mb-10">
              Expert articles on studying abroad, university applications, visas, scholarships, and student life.
            </p>
            <div className="max-w-lg mx-auto relative">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-muted-foreground" />
              <Input value={search} onChange={e => setSearch(e.target.value)}
                placeholder="Search articles..." className="pl-12 py-5 rounded-full shadow-md" />
            </div>
          </motion.div>
        </div>
      </section>
            ...
            {categories.map(c => (
              <button key={c} onClick={() => setCategory(c)}
                className={`px-5 py-2 rounded-full text-sm font-medium transition-all
                  ${category === c ? 'bg-accent text-white shadow-md shadow-accent/25' : 'bg-secondary hover:bg-accent/10 text-muted-foreground hover:text-accent'}`}>
                {c}
              </button>
            ))}
          </div>
