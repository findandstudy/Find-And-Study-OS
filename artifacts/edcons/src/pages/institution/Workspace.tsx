...
function Offers({ context }: { context?: Context }) {
  ...
  const enrol=useApiMutation((caseId)=>`/api/institution/applications/${caseId}/enrolment`);
  if(query.isLoading)return <Loading/>;if(query.error)return <ErrorState error={query.error}/>;
  return <Card><CardContent className="pt-6">{!query.data?.data?.length?<Empty/>:<Table><TableHeader><TableRow><TableHead>Applicant</TableHead><TableHead>State</TableHead><TableHead>Deadline</TableHead><TableHead>Issued</TableHead><TableHead/></TableRow></TableHeader><TableBody>{query.data.data.map((item)=><TableRow key={item.id}><TableCell>{item.masked_student_ref}</TableCell><TableCell><Status value={item.state}/></TableCell><TableCell>{formatDate(item.acceptance_deadline)}</TableCell><TableCell>{formatDate(item.issued_at)}</TableCell><TableCell className="text-right"><div className="flex justify-end gap-2">{item.state==="DRAFT"&&context?.capabilities.includes("institution.offers.issue")&&<Button size="sm" onClick={()=>window.confirm("Issue this approved offer? External delivery remains disabled.")&&issue.mutate({value:item.id,body:{}})}>Issue offer</Button>}{item.state==="ISSUED"&&context?.capabilities.includes("institution.enrolment.confirm")&&<Button size="sm" variant="outline" onClick={()=>{const hash=window.prompt("Verified enrolment evidence SHA-256");if(hash)enrol.mutate({value:item.application_case_id,body:{state:"CONFIRMED",evidenceRefHash:hash}});}}>Confirm enrolment</Button>}</div></TableCell></TableRow>)}</TableBody></Table>}</CardContent></Card>;
}
...
function Requirements() {
  ...
  const [programId,setProgramId]=useState(""),[intakeKey,setIntakeKey]=useState(""),[title,setTitle]=useState("");
