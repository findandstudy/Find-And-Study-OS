...
test("0073 permits only typed pending and reconciled commit audit states", () => {
  const migration = readFileSync(
    new URL(
      "../../../lib/db/drizzle/0073_change_set_commit_reconciliation.sql",
      import.meta.url,
    ),
    "utf8",
  );
...
test("0072 durable audit adapter is tenant-bound, append-only, and default-unwired", () => {
  const migration = readFileSync(
    new URL(
      "../../../lib/db/drizzle/0072_change_set_durable_audit_adapter.sql",
      import.meta.url,
    ),
    "utf8",
  );
...
test("idempotency migration forces RLS and allows only one-way completion", () => {
  const migration = readFileSync(
    new URL(
      "../../../lib/db/drizzle/0068_change_set_command_idempotency.sql",
      import.meta.url,
    ),
    "utf8",
  );
...
test("0069 binds tenant branches, memberships, policy, and single-use evidence", () => {
  const migration = readFileSync(
    new URL(
      "../../../lib/db/drizzle/0069_authorization_control_plane_db_hardening.sql",
      import.meta.url,
    ),
    "utf8",
  );
