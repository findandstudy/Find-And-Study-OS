...
const ID = {
  ...
  afterWithdrawalAssessment: "018f9500-0000-7000-8000-000000000016",
...
await admin.query(`GRANT USAGE ON SCHEMA public TO fas_institution_executor;
  GRANT SELECT ON TABLE institution_application_cases,institution_evidence_share_receipts TO fas_institution_executor;
...
await admin.query(`
  ...
  VALUES ('${ID.relationship}','${ID.tenant}',${LEGACY.university},'admissions.review',ARRAY['application.profile','application.evidence'],'ACTIVE');
  ...
  INSERT INTO journey_requirement_items (
    ...
    'passport','passport',true,1,'${HASH.c}'
  );
  ...
  INSERT INTO journey_verified_evidence_receipts (
    ...
    '${ID.dossierRevision}','${ID.dossier}','${ID.requirementSet}','passport',
  ...
  INSERT INTO journey_requirement_results (
    ...
    '${ID.requirementSet}','passport','VERIFIED','${ID.evidenceReceipt}',
...
await admin.query(`INSERT INTO institution_application_cases (
  ...
  $1,$2,$3,$4,$5,$6,'Fall','STU-0123456789ABCDEF','{}'::jsonb,'RECEIVED',0,
...
after(async () => {
  ...
  await admin.query(`TRUNCATE TABLE
    institution_evidence_share_receipts,institution_evidence_assessments,
    institution_application_cases,institution_memberships,institution_relationships,
  ...
  await admin.query(`DELETE FROM portal_submissions WHERE id=${portalSubmissionId};
    ...
    DELETE FROM principals WHERE id='${ID.principal}';
...
test("share executor is EXECUTE-only and function owner is non-privileged", async () => {
  ...
  assert.equal(functions.rowCount, 2);
...
test("active consent creates one PII-minimized receipt under concurrency", async () => {
  ...
  assert.equal(persisted.rows[0].requirement_code, "passport");
...
test("intake case assessment accepts only the exact active share receipt", async () => {
  ...
  await actor.query(`INSERT INTO institution_evidence_assessments (
    ...
    assessed_at
  ) VALUES ($1,$2,$3,$4,$5,$6,'VERIFIED','PASSPORT_REVIEWED',$7,$8,
    '2000-01-01T00:00:00Z')`,
  [ID.assessment, ID.tenant, ID.relationship, ID.institutionCase, share.id,
    share.evidence_ref_hash, ID.institutionMembership, HASH.b]);
...
test("withdrawal invalidates the receipt for every new assessment", async () => {
  ...
  await assert.rejects(actor.query(`INSERT INTO institution_evidence_assessments (
    id,tenant_id,relationship_id,application_case_id,evidence_share_receipt_id,
    evidence_ref_hash,result,reason_code,reviewer_membership_id,assessment_hash
  ) VALUES ($1,$2,$3,$4,$5,$6,'REJECTED','AFTER_WITHDRAWAL',$7,$8)`,
  [ID.afterWithdrawalAssessment, ID.tenant, ID.relationship, ID.institutionCase,
    share.id, share.evidence_ref_hash, ID.institutionMembership, HASH.d]),
