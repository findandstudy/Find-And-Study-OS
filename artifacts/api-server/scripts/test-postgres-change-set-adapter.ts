...
assert.equal(
  process.env.ALLOW_DISPOSABLE_ADAPTER_TEST,
  "true",
...
for (const [value, expectedUser] of [
  ...
  [executorUrl, "fas_cp_executor"],
  [evidenceIssuerUrl, "fas_evidence_issuer"],
      ...
      await migrator.query(
        `INSERT INTO public.principals
         ...
         VALUES
          ...
          ($3, 'SERVICE', 'adapter-gate', 'evidence-issuer', 'ACTIVE', 'NORMAL')`,
        [ID.humanPrincipal, ID.unboundPrincipal, ID.evidencePrincipal],
      );
      await migrator.query(
