...
function resolveTarget(result: SubmitResult | null): WritebackTarget {
  ...
  if (result.programMissing) {
    return { submissionStatus: "program_missing", stageKey: "documents" };
  }
  if (result.alreadyExists) {
    return { submissionStatus: "already_exists", stageKey: "already_registered" };
  }
  // Nothing matched — treat as failure, leave application in current stage
  return { submissionStatus: "failed", stageKey: null };
}
 ...
 * @param meta          Extra metadata to merge into result_json
 */
export async function writebackResult(
  ...
  await db
    ...
    .set({
      ...
      error:          submissionStatus === "failed" ? (errorMessage ?? "submission failed") : null,
      lockedAt:       null,
      lockedBy:       null,
      updatedAt:      new Date(),
    })
